...
A1: SELECT name from Facilities WHERE NOT membercost=0.0;

/* Q2: How many facilities do not charge a fee to members? */
A2: SELECT COUNT(name) from Facilities WHERE membercost=0.0;

/* Q3: Write an SQL query to show a list of facilities that charge a fee to members,
where the fee is less than 20% of the facility's monthly maintenance cost.
Return the facid, facility name, member cost, and monthly maintenance of the
facilities in question. */
A3: SELECT facid, name, membercost,monthlymaintenance from Facilities where membercost<(20/100*monthlymaintenance);
...
the member name. */
A7: SELECT sub.court, CONCAT( sub.firstname,  ' ', sub.surname ) AS name
FROM (
...
A8: SELECT Facilities.name AS facility, CONCAT( Members.firstname,  ' ', Members.surname ) AS name, 
...
A11: SELECT m.firstname, m.surname, r.firstname, r.surname
FROM Members AS m
